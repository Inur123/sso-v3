...
export default function SessionsPage() {
  ...
  const handleRevokeClient = async (id: string) => {
    if (!confirm("Apakah Anda yakin ingin memutuskan (cabut akses) aplikasi ini?"))
      return;
    if (!session?.session?.token) return;
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
          <Card className="border border-slate-200 bg-white shadow-sm overflow-hidden">
            <CardHeader className="flex flex-row items-center space-x-3 border-b border-slate-100 bg-slate-50/50 py-4">
              ...
              <div>
                <CardTitle className="text-sm font-semibold">Aplikasi Terhubung & Sesi SSO Aktif</CardTitle>
                <CardDescription className="text-xs text-slate-500">
                  Daftar aplikasi klien SSO yang sedang aktif diakses menggunakan akun Anda. Anda dapat mencabut otorisasi kapan saja.
                </CardDescription>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              {loadingLogs ? (
                ...
                <div className="py-16 text-center text-slate-500 text-sm flex flex-col items-center justify-center space-y-2">
                  ...
                  <p className="font-semibold text-slate-600 mt-2">Belum ada aplikasi yang terhubung.</p>
                  <p className="text-xs text-slate-400">Silakan login ke aplikasi client menggunakan SSO ini terlebih dahulu.</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-slate-200 bg-slate-50/50">
                      <TableHead className="font-semibold text-slate-700 text-xs">Nama Aplikasi</TableHead>
                      <TableHead className="font-semibold text-slate-700 text-xs">Key / Token Otorisasi</TableHead>
                      <TableHead className="font-semibold text-slate-700 text-xs">Waktu Login SSO</TableHead>
                      <TableHead className="font-semibold text-slate-700 text-xs">Status Sesi</TableHead>
                      <TableHead className="text-right font-semibold text-slate-700 text-xs">Aksi</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {ssoLogs.map((log) => {
                      const isTokenVisible = !!visibleTokenIds[log.id];
                      // Sensor token: tampilkan lengkap jika tombol mata diklik, jika tidak sensor 16 karakter di belakang
                        ...
                        <TableRow key={log.id} className="border-b border-slate-100 hover:bg-slate-50/50 text-xs">
                          ...
                          <TableCell className="font-mono text-slate-600">
                            <div className="flex items-center space-x-2">
                              <code className="rounded bg-slate-100 px-2 py-1 text-[11px] text-slate-800">
                                {displayToken}
                              </code>
                              <Button
                                variant="ghost"
                                ...
                                title={isTokenVisible ? "Sembunyikan Key" : "Tampilkan Key"}
                              >
                                {isTokenVisible ? (
